      ...
      it "always produces correct magic number for any valid font bbox" do
        100.times do
          ...
          expect(magic).to eq(0x5F0F3CF5),
                           "Magic number should always be 0x5F0F3CF5"
        end
      end
      ...
      it "always uses 1000 units per em for Type 1 fonts" do
        100.times do
          ...
          expect(upem).to eq(1000),
                          "Units per em should always be 1000 for Type 1 fonts"
        end
      end
    end
      ...
      it "always produces version 0.5 for any glyph count" do
        100.times do
          ...
          expect(version).to eq(0x00005000),
                             "Maxp version should always be 0.5 for CFF fonts"

          # Verify glyph count is preserved
          stored_count = maxp_data[4..5].unpack1("n")
          expect(stored_count).to eq(num_glyphs),
                                  "Glyph count should be preserved"
        end
      end
    end
      ...
      it "always produces weight class in valid range" do
        ...
        weight_names.each do |weight_name|
          ...
          expect(weight_class).to be >= 100,
                                  "Weight class should be at least 100"
          expect(weight_class).to be <= 900,
                                  "Weight class should be at most 900"
        end
      end
    end
    ...
    context "name table encoding invariant" do
      it "always uses Windows platform ID 3" do
        100.times do
          ...
          expect(platform_id).to eq(3),
                                 "Platform ID should always be Windows (3)"
        end
      end
      ...
      it "always uses Unicode BMP encoding ID 1" do
        100.times do
          ...
          expect(encoding_id).to eq(1),
                                 "Encoding ID should always be Unicode BMP (1)"
        end
      end
      ...
      it "always uses US English language ID 0x0409" do
        100.times do
          ...
          expect(language_id).to eq(0x0409),
                                 "Language ID should always be US English (0x0409)"
        end
      end
    end
      ...
      it "always produces version 3.0 for any font" do
        100.times do
          ...
          expect(version).to eq(0x00030000),
                             "Post table version should always be 3.0 for CFF fonts"
        end
      end
    ...
    context "cmap table encoding invariant" do
      it "always uses Windows platform (3) and Unicode BMP encoding (1)" do
        100.times do
          ...
          expect(platform_id).to eq(3),
                                 "Cmap should always use Windows platform"
          expect(encoding_id).to eq(1),
                                 "Cmap should always use Unicode BMP encoding"
        end
      end
    end
  end
      ...
      it "always uses valid default values" do
        100.times do
          ...
          expect(result[:blue_scale]).to eq(0.039625),
                                         "Blue scale should have default value"
          expect(result[:blue_shift]).to eq(7),
                                         "Blue shift should have default value"
          expect(result[:blue_fuzz]).to eq(1),
                                        "Blue fuzz should have default value"
          expect(result[:force_bold]).to be(false),
                                         "Force bold should default to false"
        end
      end
    end
  end
